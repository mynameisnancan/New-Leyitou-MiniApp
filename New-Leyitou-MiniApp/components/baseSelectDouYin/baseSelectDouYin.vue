		...
		<view class="uni-p-lg">
			...
			<scroll-view scroll-y class="hide-scrollbar" style="height: 600rpx;" :lower-threshold="100"
				...
				<wd-checkbox-group v-model="selectedValue" v-if="multiple" @change="multipleChange">
					<wd-checkbox :modelValue="item.authorId" v-for="(item,index) in listData" :key="index"
						shape="square">
						<view class="uni-flex uni-px-lg uni-items-center underline uni-w-full">
							<view>
								<wd-img v-if="item?.dyAuthorInfo?.avatar" width="120rpx" height="120rpx" round
				...
				<wd-loadmore v-if="noData && listData.length>0" state="finished" :loading-props="{size:'40rpx'}"
...
<style lang="scss" scoped>
	:deep(.wd-radio) {
		border-bottom: 1px solid var(--wot-dark-color3, rgba(232, 230, 227, .8));;
	}
</style>
